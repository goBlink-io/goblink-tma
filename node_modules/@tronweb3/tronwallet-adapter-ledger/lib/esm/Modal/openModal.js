...
export function openConnectingModal() {
    ...
    render(_jsx(Modal, { title: langText.loadingTitle, onClose: onClose, children: _jsx(ConnectingContent, {}) }), div);
    return onClose;
}
export function openVerifyAddressModal(address) {
    const { onClose, div } = prepareDomNode();
    const langText = getLangText();
    render(_jsx(Modal, { width: 550, title: langText.loadingTitle, onClose: onClose, children: _jsx(ConfirmContent, { address: address }) }), div);
    return onClose;
}
export function openSelectAccountModal(options) {
    ...
    return new Promise((resolve, reject) => {
        ...
        render(_jsx(Modal, { title: langText.loadingTitle, onClose: onCancel, children: _jsx(SelectAccount, { accounts: options.accounts, selectedIndex: options.selectedIndex || 0, onConfirm: onConfirm, onCancel: onCancel, getAccounts: options.getAccounts }) }), div);
    });
}
